...
def submit_data():
    ...
    room_price = room_prices.get(room_type, 0)  # Default to 0 if room type is invalid
    ...
    if result > 0:
        ...
        messagebox.showinfo("Result", "Error inserting record.")
...
def fetch_data():
    ...
    db = sql.connect(host='localhost', user='root', password='Saloni@123', db='project1')
    cur = db.cursor()
    query = "SELECT customer_contact, check_in_date, check_out_date, room_type, room_price FROM records WHERE customer_contact = %s"
    cur.execute(query, (customer_contact,))
    result = cur.fetchall()
    ...
    if result:
        for col in result:
            ...
            room_type = col[3]
            room_price = col[4]  # Get room price

            tv.insert("", 'end', values=(customer_contact, check_in_date, check_out_date, room_type, room_price))
    else:
...
def show():
    ...
    db = sql.connect(host='localhost', user='root', password='Saloni@123', db='project1')
    cur = db.cursor()
    query = "SELECT customer_contact, check_in_date, check_out_date, room_type, room_price FROM records"
    cur.execute(query)
    result = cur.fetchall()
...
cal1 = DateEntry(top, width=30, bg='darkblue', fg='white')
cal1.place(x=250, y=265)

l3 = Label(f2, text='Check_out Date   :-', font=('Arial 10 bold'))
l3.place(x=10, y=80)

cal2 = DateEntry(top, width=30, bg='darkblue', fg='white')
...
b5 = Button(top, text='   EXIT    ', bg='tan', font=('Arial 15'), highlightbackground='black', highlightthickness=1,
            command=exit)
b5.place(x=450, y=70)
